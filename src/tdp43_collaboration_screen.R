...
for (i in 1:4) {
  ...
  het_snps <- setdiff(het_snps, readLines("src/indel_snps.txt")) # Update SNPs to exclude indels
  input = input[(input$variantID %in% het_snps),]
  res[[i]] <- c(res[[i]],  nrow(input))
...
filtered_input_files <- str_replace(input_files, "out$", paste0("10readsInput_filtered_epsilon",epsilon,"_sharedhet.txt"))
...
plot(density(ip_data$totalCount))
